...
const JapanOrderUpdate = ({ errMsg, projectType, isEdit, id }) => {
    ...
    useEffect(() => {
         if(projectCreateJPOrderState?.jpDeliveryDate) {
            const deliveryDate = new Date(projectCreateJPOrderState?.jpDeliveryDate)
            setDeliveryDate(parseISO(deliveryDate.toISOString()))
        }
        if(projectCreateJPOrderState?.jpAcceptanceBillingDate) {
            const acceptanceDate = new Date(projectCreateJPOrderState?.jpAcceptanceBillingDate)
            setAcceptanceDate(parseISO(acceptanceDate.toISOString()))
        }
        if(projectCreateJPOrderState?.jpPaymentDate) {
            const paymentDate = new Date(projectCreateJPOrderState?.jpPaymentDate)
    ...
    }, [filteredLeaders, projectCreateJPOrderState])

    const {formatDate} = useInputValidation()

    return (
        <div
            className="flex flex-col justify-center gap-x-20 rounded-md pb-4 pt-8"
                ...
                <div className="mb-4 w-full space-y-2">
                    ...
                    <DatePicker
                        ...
                        {projectType === 'japan-order' ? (
                            ...
                            <div className="mr-8 flex w-full justify-start gap-4">
                                <Link
                                    href={`/en/projects/${id}/japan-myanmar-order/myanmar-info`}
                                >
                                    <button
...
export default JapanOrderUpdate
