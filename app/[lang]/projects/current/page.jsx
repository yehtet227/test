...
import { useGetCurrentProjects } from '@/app/store/server/features/projects'
...
const Page = () => {
    ...
    const { data: currentProjects, isLoading, isError, isSuccess, error } = useGetCurrentProjects()
    function getProjectProperty(project, sesProp, jpProp, mmProp, isJPMM) {
        ...
        } else if (project?.orders[0]?.project_type?.project_type.includes('Offshore(Japan)')) {
            return project?.orders[0]?.[jpProp];
        } else if (project?.orders[0]?.project_type?.project_type.includes('Offshore(Myanmar)')) {
            return project?.orders[0]?.[mmProp];
        } else {
            if(isJPMM) {
                return Number(project?.orders[0]?.[jpProp]) + Number(project?.orders[0]?.[mmProp]);
            } else {
                return project?.orders[0]?.[jpProp];
            }
        }
    ...
    let projectLists
    if (currentProjects) {
        projectLists = currentProjects?.map((project, index) => ({
            key: project?.id,
            ...
            deliveryDate: getProjectProperty(project, 'ses_delivery_date', 'jp_delivery_date', 'mm_delivery_date'),
            acceptanceDate: getProjectProperty(project, 'ses_acceptance_billing_date', 'jp_acceptance_billing_date', 'mm_acceptance_billing_date'),
            paymentDate: getProjectProperty(project, 'ses_payment_date', 'jp_payment_date', 'mm_payment_date'),
            contractNumber: project?.contract_number,
            estimateNumber: getProjectProperty(project, 'ses_estimate_number', 'jp_estimate_number', 'mm_estimate_number'),
            orderNumber: getProjectProperty(project, 'ses_order_number', 'jp_order_number', 'mm_order_number'),
            estimateCost: getProjectProperty(project, 'ses_order_amount', 'jp_order_amount', 'mm_order_amount', 'jpmm'),
        }))
    }
